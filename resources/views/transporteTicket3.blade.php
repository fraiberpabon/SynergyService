        ...
        <body class="container mt-2" style="max-width: 1140px;">
            ...
            @if (isset($transport))
                ...
                <div class="justify-content-center">
                    <x-bladewind::timeline-group stacked="false" anchor="big" completed="true" color="blue"
                                            ...
                                            <x-bladewind::card>
                                                ...
                                                @else
                                                    <div class="row">
                                                        ...
                                                        <div class="col">
                                                            <p>{{ $solicitudes = collect($transport)->pluck('solicitud')->get(0) }}
                                                            </p>
                                                        </div>
                                                    </div>
                                                @endif
                    ...
                    @foreach ($transport as $key)
                        @php
                            if (isset($key['tipo']) && $key['tipo'] == '2' && isset($key['equipo'])) {
                                $salida = $key['plantaOrigen'] ? "<p>{$key['plantaOrigen']}</p>" : '<p>' . __('messages.frente_card') . (!empty($key['tramoOrigen']) ? $key['tramoOrigen'] : __('messages.default_tramo')) . '<br>' . __('messages.zona_card') . (!empty($key['hitoOrigen']) ? $key['hitoOrigen'] : __('messages.default_hito')) . (!empty($key['abscisaOrigen']) ? '<br>' . __('messages.abscisa_card') . ' ' . $key['abscisaOrigen'] : '') . '</p>';
                            ...
                            if (isset($key['tipo']) && $key['tipo'] == '1' && isset($key['equipo'])) {
                                $salidas_info = $key['plantaDestino'] ? "<p>{$key['plantaDestino']}</p>" : '<p>' . __('messages.frente_card') . (!empty($key['tramoDestino']) ? $key['tramoDestino'] : __('messages.default_tramo')) . '<br>' . __('messages.zona_card') . (!empty($key['hitoDestino']) ? $key['hitoDestino'] : __('messages.default_hito')) . (!empty($key['abscisaDestino']) ? '<br>' . __('messages.abscisa_card') . ' ' . $key['abscisaDestino'] : '') . '</p>';
